...
int main(void)
{
	...
	printf("\nAfter 2 seconds,\nyou should see all the LEDs flicker up randomly.\n");
	printf("Every switch you toggle must give a response in the printed output.\n");
	printf("\nThe printed output shows 5 groups of octal numbers,\n");
	printf("each octal number represents 3 switches. 0 indicates all switches off,\n");
	printf("and so with all switches off you must start with seeing all 5 groups as 0.\n"); 

	printf("\nTest that all lamps light up, and all switches generate a response.\n");
	printf("The program runs for 120 seconds, or ^C to abort\n\n");
	
	// start up multiplexing thread
	gpio_mux_thread_start();

	double RUNTIME=120.0;
